...
CREATE TABLE CHAR_COMPARE (
    sn CHAR(10),     -- -> 7개의 공백이 생김. <-CHAR(10) 
    char_4 CHAR(4),
    char_6 CHAR(6) --> 공백 2칸.
);
--정확하게 자릿수가 정해져있을 때 이용함 EX) 주민번호, YorN 선택시
...
SELECT 
    ...
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
WHERE sn = '101'       -- WHERE절이 참이어야 SELECT를 조회함. 조회가 됨. char_4 = char_6 참O => CHAR 타입끼리의 비교에서는 공백을 무시. 글자만 같으면 같다고함.
    AND char_4 = char_6
;
...
INSERT INTO varchar_compare VALUES ('101', 'SQLD', 'SQLD  ');  -- 이렇게 의도적으로 공백을 넣어줘야 공백이 들어감. 
...
WHERE sn = '101'
    AND char_4 = varchar_6 -- 조회 안됨. ->둘이 다르다. =  VARCHAR의 공백을 무시X
;
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD' -- 조회안됨.  
;
-- => 상수는 비교 대상(char_4) (varchar_6)의 변수(CHAR) (VARCHAR)를 따라간다
...
SELECT 
    ...
    SUBSTR('SQL Developer', 5, 3), -- 문자열 잘라서 추출. 1번부터 3개자름 (첫글자가 1번)
...
FROM dual;
 -- ex) 도서 대출 시스템, 3일 후 반납해야함. 대출 시간 sysdate, 반납시간 : sysdate+ 3
...
SELECT
    ...
    TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') AS "날짜 - 한글포함",  -- 한글은 " "(쌍따옴표) 안에
    TO_CHAR(SYSDATE + (1/24) * 6, 'YY/MM/DD HH24:MI:SS') AS "날짜와 시간",
    TO_CHAR(SYSDATE + (1/24) * 6, 'YY/MM/DD AM HH12:MI:SS') AS "날짜와 시간2"  -- AM만 써도 오전 오후 알아서 해줌.
FROM dual;
...
-- 문자를 숫자로 변환
SELECT 
    TO_NUMBER('$5,500', '$999,999') - 4000 AS "계산결과"   -- $5500 - 4000 은 원래 안됨.
FROM dual;
...
SELECT 
    NVL(MAX(emp_nm), '존재안함') AS emp_nm  -- MAX안쓰면 아무것도 안뜸. (박찬호는 공집합이기 때문. 데이터가 없음)
FROM tb_emp                                             -- 공집합의 최댓값 (MAX) = NULL을 뱉어냄. NVL 도구로 사용함.
WHERE emp_nm = '박찬호';
...
FROM tb_emp
WHERE emp_nm = '김회장'                           --> direct_manager_emp_no가 있는것임. null이라고 있는거임(미정). 無가 아님.
;                                                               --count(*) 하면 1
SELECT
    direct_manager_emp_no
FROM tb_emp
WHERE emp_nm = '박찬호'                          --> 아무것도 안나옴. 공집합임. 無.
;                                                               --count(*) 하면 0
